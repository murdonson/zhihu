...
*/
    ...
    public void afterPropertiesSet() throws Exception {
        pool = new JedisPool("redis://localhost:6379/10");//启动redis服务
    }
    ...
    public long scard(String key) {
        ...
        try {
            jedis = pool.getResource();
            return jedis.scard(key);//SCARD命令用于获取存储在集合中的元素的数量 集合里的元素具有唯一性
        } catch (Exception e) {
            logger.error("发生异常" + e.getMessage());
        } finally {
            if (jedis != null) {
    ...
    public List<String> brpop(int timeout, String key) {
        ...
        try {
            jedis = pool.getResource();
            return jedis.brpop(timeout, key);//假如在指定时间内没有任何元素被弹出，则返回一个 nil 和等待时长。
            // 反之，返回一个含有两个元素的列表，第一个元素是被弹出元素所属的 key ，第二个元素是被弹出元素的值。
            //Redis Brpop 命令移出并获取列表list的最后一个元素， 如果列表没有元素会阻塞列表直到等待超时或发现可弹出元素为止。
        } catch (Exception e) {
            logger.error("发生异常" + e.getMessage());
        } finally {
            if (jedis != null) {
    ...
    public long lpush(String key, String value) {
        ...
        try {
            jedis = pool.getResource();
            return jedis.lpush(key, value);//Redis Lpush 命令将一个或多个值插入到列表list头部。
